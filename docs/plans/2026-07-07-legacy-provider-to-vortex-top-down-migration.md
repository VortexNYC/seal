...
- `bun run audit:seal-vortex-sandbox-settlement-boundary` statically verifies the captured sandbox payment ids, the Vortex final sandbox launch gate, and the human-run settlement commands without touching Convex, Finix, reconciliation, payouts, or card payment.
