...
describe("createDisputesHttpHandlers", () => {
  ...
  test("returns success responses for dispute actions", async () => {
    ...
    await expect(handlers.acceptDispute({
      body: { environment: "sandbox", merchantAccountId: "merchant_123", disputeId: "dp_123" },
    })).resolves.toMatchObject({ status: 200, body: { data: { dispute: { responseState: "accepted" } } } });
    await expect(handlers.createDisputeEvidence({
      body: {
        environment: "sandbox",
        merchantAccountId: "merchant_123",
        disputeId: "dp_123",
    ...
    await expect(handlers.listDisputeEvidence({
      body: { environment: "sandbox", merchantAccountId: "merchant_123", disputeId: "dp_123" },
    })).resolves.toMatchObject({ status: 200, body: { data: { items: [{ state: "PENDING" }] } } });
    await expect(handlers.submitDisputeEvidence({
      body: {
        environment: "sandbox",
        merchantAccountId: "merchant_123",
        disputeId: "dp_123",
    ...
    })).resolves.toMatchObject({ status: 200, body: { data: { dispute: { evidenceStatus: "submitted" } } } });
    await expect(handlers.listDisputeAdjustmentTransfers({
      body: { environment: "sandbox", merchantAccountId: "merchant_123", disputeId: "dp_123" },
    })).resolves.toMatchObject({ status: 200, body: { data: { items: [{ amount: 900, currency: "USD" }] } } });
  });
});
