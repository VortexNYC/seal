...
export function registerUploadTools(server: McpServer, client: SealApiClient): void {
      ...
      if (!stats.isFile()) {
        return {
          content: [
            {
              type: "text" as const,
              text: JSON.stringify({ error: `Path is not a file: ${file_path}` }, null, 2),
            },
          ],
          isError: true,
        };
  ...
  server.tool(
    ...
    async (args, extra) => {
      ...
      try {
        ...
        return {
          content: [
            {
              type: "text" as const,
              text: JSON.stringify({ error: "Invalid base64 encoding" }, null, 2),
            },
          ],
          isError: true,
        };
      ...
      const storageId = await client.uploadToStorage(upload_url, fileBuffer, "application/pdf");
