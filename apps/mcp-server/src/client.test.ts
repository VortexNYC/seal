...
function createMockFetch(
  handler: (url: string | URL | Request, options?: RequestInit) => Promise<Response>,
...
describe("SealApiClient", () => {
  ...
  describe("get", () => {
    ...
    test("throws error when no auth credentials available", async () => {
      ...
      expect(client.get("/documents")).rejects.toThrow("Missing API credentials");
    });
  });
  ...
  describe("put", () => {
    test("makes PUT request", async () => {
      ...
      const client = new SealApiClient(mockConfig);
      await client.put("/documents/update", { title: "Updated" }, { id: "123" }, "token");
  ...
  describe("error handling", () => {
    ...
    test("handles RFC 7807 error format", async () => {
      ...
      try {
        await client.post("/recipients", { email: "invalid" }, undefined, "token");
  ...
  describe("uploadToStorage", () => {
    ...
    test("throws error on upload failure", async () => {
      ...
      expect(
        client.uploadToStorage("https://storage.example.com/upload", buffer, "application/pdf"),
  ...
  describe("uploadToStorageStream", () => {
    test("uploads file stream to storage URL", async () => {
      ...
      globalThis.fetch = createMockFetch(async (url, options) => {
        capturedUrl = url as string;
        capturedOptions = options as RequestInit;
        return new Response(JSON.stringify({ storageId: "storage-stream-123" }), {
          status: 200,
        });
      });
      ...
      expect((capturedOptions!.headers as Record<string, string>)["Content-Length"]).toBe("18");
