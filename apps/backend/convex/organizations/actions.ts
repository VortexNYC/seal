...
export const getInvitationEmailByClerkId = action({
  ...
  handler: async (ctx, args): Promise<{ email: string | null }> => {
    ...
    try {
      ...
      const invitation = await ctx.runQuery(internal.organizations.queries.getInvitationByClerkId, {
...
export const clerkResendInvitation = action({
  ...
  handler: async (ctx, args): Promise<{ ok: boolean; message: string }> => {
    ...
    const invitation = await ctx.runQuery(internal.organizations.queries.getInvitationById, {
