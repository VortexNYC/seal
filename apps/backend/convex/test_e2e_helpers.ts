...
export const prepareBillingCutoverWorkspaceForE2E = mutation({
  args: {
    organizationSlug: v.string(),
    lookupKey: v.string(),
  },
  handler: async (ctx, { organizationSlug, lookupKey }) => {
    requireE2eDeployment();
    ...
    if (!org) {
      return { prepared: false, reason: "org_not_found" };
    }
    ...
    if (!price) {
      return { prepared: false, reason: "price_not_found", organizationId: org._id };
    }
    ...
    for (const subscription of subscriptions) {
      if (subscription.status === "active" || subscription.status === "trialing") {
        await ctx.db.patch(subscription._id, {
          status: "canceled",
          canceledAt: now,
          cancelReason: "vortex_billing_cutover_e2e_reset",
          updatedAt: now,
        });
    ...
    return {
      ...
      stripeCustomerIdPresent: org.stripeCustomerId !== undefined,
    };
  },
});

export const prepareBillingCutoverActiveUserForE2E = mutation({
  args: {
    email: v.string(),
    lookupKey: v.string(),
  },
  handler: async (ctx, { email, lookupKey }) => {
    requireE2eDeployment();
    ...
    if (users.length === 0) {
      return { prepared: false, reason: "user_not_found" };
    }
    ...
    if (!price) {
      return { prepared: false, reason: "price_not_found" };
    }

    const now = Date.now();
    const preparedOrganizations: Array<{
      userId: Id<"users">;
    ...
    if (preparedOrganizations.length === 0) {
      return { prepared: false, reason: "active_organization_not_found" };
    }

    const primary = preparedOrganizations[0]!;
    return {
      prepared: true,
      userId: primary.userId,
      organizationId: primary.organizationId,
      organizationSlug: primary.organizationSlug,
      organizationIds: preparedOrganizations.map((org) => org.organizationId),
      organizations: preparedOrganizations,
      lookupKey,
      stripePriceId: price.externalPriceId,
      canceledSubscriptionCount: preparedOrganizations.reduce(
        (sum, org) => sum + org.canceledSubscriptionCount,
        0,
      ),
      stripeCustomerIdPresent: preparedOrganizations.some((org) => org.stripeCustomerIdPresent),
    };
  },
});

function archivedBillingCutoverLookupKey(priceId: Id<"subscription_prices">, lookupKey: string) {
  return `archived:${priceId}:${lookupKey}`;
}

async function patchBillingCutoverCanonicalPrice(
  ctx: MutationCtx,
  input: {
    productId: Id<"subscription_products">;
    priceId: Id<"subscription_prices">;
    productMetadata: { tier?: string; useType?: string; features?: string } | undefined;
    lookupKey: string;
  ...
  await ctx.db.patch(input.productId, {
    externalProductId: input.stripeProductId,
    status: "active",
    metadata: {
  ...
  await ctx.db.patch(input.priceId, {
    externalProductId: input.stripeProductId,
    externalPriceId: input.stripePriceId,
    status: "active",
...
async function patchBillingCutoverLookupDuplicates(
  ctx: MutationCtx,
  input: {
    canonicalPriceId: Id<"subscription_prices">;
    lookupKey: string;
    stripePriceId: string;
    stripeProductId: string;
    now: number;
  ...
  let patchedLookupPriceCount = 0;
  const lookupPrices = await ctx.db
    .query("subscription_prices")
    .withIndex("by_lookup_key", (q) => q.eq("lookupKey", input.lookupKey))
    .take(100);
  for (const lookupPrice of lookupPrices) {
    const isCanonical = lookupPrice._id === input.canonicalPriceId;
    await ctx.db.patch(lookupPrice._id, {
      externalProductId: input.stripeProductId,
      externalPriceId: input.stripePriceId,
      status: isCanonical ? "active" : "archived",
      lookupKey: isCanonical
        ? input.lookupKey
        : archivedBillingCutoverLookupKey(lookupPrice._id, input.lookupKey),
      updatedAt: input.now,
    });
    patchedLookupPriceCount += 1;
  }
  return patchedLookupPriceCount;
}

async function archiveBillingCutoverCompetingProducts(
  ctx: MutationCtx,
  canonicalProductId: Id<"subscription_products">,
  now: number,
) {
  ...
  const activeProducts = await ctx.db
    ...
    .take(200);
  for (const candidateProduct of activeProducts) {
    const isCompetingProProduct =
      candidateProduct._id !== canonicalProductId &&
      (candidateProduct.metadata?.tier === "pro" ||
        candidateProduct.externalProductId.startsWith("vtx_prod_"));
    if (!isCompetingProProduct) {
      continue;
    }
    await ctx.db.patch(candidateProduct._id, {
      status: "archived",
      updatedAt: now,
    });
    archivedProductCount += 1;
  }
  return archivedProductCount;
}

async function repairBillingCutoverCompetingPrices(
  ctx: MutationCtx,
  input: {
    canonicalPriceId: Id<"subscription_prices">;
    lookupKey: string;
  ...
  let repairedVortexPriceCount = 0;
  let archivedPriceCount = 0;
  const activePrices = await ctx.db.query("subscription_prices").take(200);
  for (const candidatePrice of activePrices) {
    const candidateProduct = await ctx.db.get(candidatePrice.subscriptionProductId);
    const isVortexPrice = candidatePrice.externalPriceId.startsWith("vtx_price_");
    if (isVortexPrice) {
      await ctx.db.patch(candidatePrice._id, {
        externalProductId: input.stripeProductId,
        externalPriceId: input.stripePriceId,
        updatedAt: input.now,
      });
      repairedVortexPriceCount += 1;
    }
    const isCompetingMonthlyPrice =
      candidatePrice._id !== input.canonicalPriceId &&
      candidatePrice.status === "active" &&
      candidatePrice.recurring?.interval === "month" &&
      (candidatePrice.lookupKey === input.lookupKey ||
        candidateProduct?.metadata?.tier === "pro" ||
        isVortexPrice);
    if (!isCompetingMonthlyPrice) {
      continue;
    }
    await ctx.db.patch(candidatePrice._id, {
      status: "archived",
      lookupKey: archivedBillingCutoverLookupKey(candidatePrice._id, input.lookupKey),
      updatedAt: input.now,
    });
    archivedPriceCount += 1;
  }
  return { repairedVortexPriceCount, archivedPriceCount };
}

export const patchBillingCutoverStripePriceForE2E = internalMutation({
  args: {
    lookupKey: v.string(),
    stripePriceId: v.string(),
    stripeProductId: v.string(),
  },
  handler: async (ctx, { lookupKey, stripePriceId, stripeProductId }) => {
    requireE2eDeployment();
    ...
    const price = await ctx.db
      ...
      .first();
    if (!price) {
      throw new Error(`price_not_found:${lookupKey}`);
    }

    const product = await ctx.db.get(price.subscriptionProductId);
    if (!product) {
      throw new Error(`product_not_found:${price.subscriptionProductId}`);
    }

    const now = Date.now();
    await patchBillingCutoverCanonicalPrice(ctx, {
    ...
    const patchedLookupPriceCount = await patchBillingCutoverLookupDuplicates(ctx, {
    ...
    return {
      ...
      stripePriceId,
      lookupKey,
      patchedLookupPriceCount,
      repairedVortexPriceCount,
...
export const ensureStripePriceForBillingCutoverE2E = action({
  args: {
    lookupKey: v.string(),
  },
  handler: async (ctx, { lookupKey }) => {
    ...
    if (!stripeSecretKey) {
      throw new Error("STRIPE_SECRET_KEY not configured on this deployment");
    }
    ...
    const price =
      ...
      (await stripe.prices.create({
        ...
        product_data: {
          name: "Seal Pro E2E Billing Cutover",
          metadata: { proof: "seal-vortex-saas-browser-cutover", tier: "pro" },
        },
        metadata: { proof: "seal-vortex-saas-browser-cutover", tier: "pro" },
      }));

    const productId = typeof price.product === "string" ? price.product : price.product.id;
    await ctx.runMutation(internal.test_e2e_helpers.patchBillingCutoverStripePriceForE2E, {
      lookupKey,
    ...
    return {
      ...
      stripePriceId: price.id,
      stripeProductId: productId,
      reused: existingPrice !== undefined,
    };
  },
});
