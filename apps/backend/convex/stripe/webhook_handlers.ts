...
async function handleCouponCreated(
	...
	try {
		hydratedCoupon = (await ctx.runAction(
			internal.stripe.actions.retrieveCoupon,
			{ couponId: coupon.id },
		)) as Stripe.Coupon;
	} catch (error) {
...
async function handlePromotionCodeCreated(
	...
	if (!stripeCouponId) {
		console.info(
			"Promotion code webhook missing coupon, fetching from Stripe",
			{
		...
		const coupon =
			typeof hydratedPromotionCode.coupon === "object"
				? hydratedPromotionCode.coupon
				: null;
		...
		await ctx.runMutation(
			internal.stripe.promo_code.handlePromotionCodeCreatedOrUpdated,
			{
...
const EVENT_HANDLERS: Record<string, EventHandler> = {
	...
	// Coupon events
	"coupon.created": (ctx, data) =>
		handleCouponCreated(ctx, data as Stripe.Coupon),
	"coupon.updated": (ctx, data) =>
		handleCouponUpdated(ctx, data as Stripe.Coupon),
	"coupon.deleted": (ctx, data) =>
		handleCouponDeleted(ctx, data as Stripe.DeletedCoupon),
