...
export const handlePromotionCodeCreatedOrUpdated = internalMutation({
  ...
  handler: async (ctx, { promotionCode, coupon }) => {
    ...
    const couponId = await ctx.runMutation(internal.stripe.coupon.handleCouponCreatedOrUpdated, {
      coupon: stripeCoupon,
    });
    ...
    const promoCodeData = {
      ...
      restrictions: stripePromoCode.restrictions
        ? {
            firstTimeTransaction: stripePromoCode.restrictions.first_time_transaction,
            minimumAmount: stripePromoCode.restrictions.minimum_amount ?? undefined,
