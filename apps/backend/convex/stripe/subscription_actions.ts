...
function isAutoEnrollEnabled(): boolean {
	...
	return Boolean(
		autoEnroll && ["1", "true", "TRUE", "yes", "on"].includes(autoEnroll),
	);
}
...
export const createSubscriptionRecord = internalMutation({
	...
	handler: async (ctx, args) => {
		...
		if (existing) {
			console.warn(
				`Subscription ${args.stripeSubscriptionId} already exists, skipping creation`,
			);
			return existing._id;
		}
...
export const subscribeUserToDefaultPlan = internalAction({
	...
	handler: async (
		...
		if (!lookupKey) {
			console.warn(
				"DEFAULT_PLAN_LOOKUP_KEY not configured, skipping auto-enrollment",
			);
			return {
				status: "skipped",
		...
		try {
			...
			if (existingSub) {
				console.warn(
					`Found existing Stripe subscription ${existingSub.id} for user ${userId}`,
				);
				return {
					status: "already_subscribed",
					subscriptionId: existingSub.id,
					stripePriceId:
						existingSub.items.data[0]?.price?.id ?? "unknown_price_id",
				};
			}
		} catch (err) {
...
export const handleNewUserSignup = internalAction({
	...
	handler: async (
		ctx,
		{ userId, email, name },
	): Promise<HandleNewUserSignupResult> => {
		const stripe = initializeStripe();
