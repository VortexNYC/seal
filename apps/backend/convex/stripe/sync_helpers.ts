...
function parseProductMetadata(product: Stripe.Product): ProductMetadata | undefined {
...
async function buildPriceParams(ctx: ActionCtx, price: Stripe.Price, productId: string) {
  ...
  return {
    ...
    status: (price.active ? "active" : "archived") as "active" | "archived" | "deleted",
...
async function syncSinglePrice(
  ...
  const priceResult = await ctx.runMutation(internal.stripe.sync.upsertPrice, priceParams);

  const amount = priceResult.amount ? `$${priceResult.amount / 100}` : "metered";
  const action = priceResult.action === "created" ? "Created" : "Updated";
  console.warn(`  ${action} price: ${price.lookup_key || price.id} (${amount})`);
}
