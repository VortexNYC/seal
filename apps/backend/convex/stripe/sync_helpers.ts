...
function parsePriceMetadata(price: Stripe.Price): PriceMetadata | undefined {
	...
	const documentsPerMonth = parseIntMetadataValue(
		metadata,
		"documentsPerMonth",
		"documents_per_month",
	);
	const maxRecipients = parseIntMetadataValue(
		metadata,
		"maxRecipients",
		"max_recipients",
	);
...
/**
 * Sync product features from Stripe Entitlements API
 *
 * @see https://docs.stripe.com/api/product-feature/list
 */
export async function syncProductFeatures(
	...
	if (!product) {
		console.warn(`  Skipping features - product ${productId} not in database`);
		return;
	}

	let startingAfter: string | undefined;
	let total = 0;
	const syncedFeatureIds: string[] = [];

	// Paginate through all features attached to this product
	while (true) {
		const params: Stripe.ProductListFeaturesParams = {
			limit: 100,
		};
		if (startingAfter) params.starting_after = startingAfter;

		const page = await stripe.products.listFeatures(productId, params);
		if (page.data.length === 0) break;

		for (const productFeature of page.data) {
			// productFeature contains entitlement_feature with the feature details
			const feature = productFeature.entitlement_feature;
			if (!feature || !feature.lookup_key) {
				console.warn(
					`  Skipping feature ${productFeature.id} - missing lookup_key`,
				);
				continue;
			}
			...
			syncedFeatureIds.push(feature.id);
			const action = result.action === "created" ? "Created" : "Updated";
			console.warn(`    ${action} feature: ${feature.lookup_key}`);
			total++;
		}
	...
	// Remove features that are no longer attached to this product
	if (syncedFeatureIds.length > 0 || total === 0) {
		const removeResult = await ctx.runMutation(
			internal.stripe.sync.removeProductFeatures,
			{
				externalProductId: productId,
				keepFeatureIds: syncedFeatureIds,
			},
		);
		if (removeResult.removed > 0) {
			console.warn(`    Removed ${removeResult.removed} detached features`);
		}
	}

	if (total > 0) {
		console.warn(`  Synced ${total} features for ${productName}`);
	}
}
