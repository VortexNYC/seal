...
export const vortexPostHog = createVortexPostHogConvexObservability(
  {
    capture: async (ctx: PostHogSchedulerCtx, args: VortexPostHogCaptureArgs) => {
      await posthog.capture(ctx, args);
    },
    identify: async (ctx: PostHogSchedulerCtx, args: VortexPostHogIdentifyArgs) => {
      await posthog.identify(ctx, args);
    },
    groupIdentify: async (ctx: PostHogSchedulerCtx, args: VortexPostHogGroupIdentifyArgs) => {
    ...
    captureException: async (ctx: PostHogSchedulerCtx, args: VortexPostHogExceptionArgs) => {
