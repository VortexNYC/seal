...
export async function ensureProFeature(
	...
	if (!isPro) {
		throw new ConvexError(
			`${featureName} requires a Pro plan. Please upgrade to continue.`,
		);
	}
}
 ...
 */
export async function ensureDocumentLimit(
	db: DatabaseReader,
	userId: Id<"users">,
): Promise<void> {
	const { isPro } = await getSubscriptionPlan(db, userId);
	const limit = isPro
		? PLAN_LIMITS.pro.documentsPerMonth
		: PLAN_LIMITS.free.documentsPerMonth;
