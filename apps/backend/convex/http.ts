 ...
 *
 * @returns List of webhook endpoints with delivery statistics
 */
http.route({
 ...
 * @bodyparam {string[]} [events] - Event types to subscribe to (empty = all)
 * @bodyparam {string} [description] - Optional description
 *
 * @returns Created endpoint with secret (shown only once)
 */
http.route({
	path: "/api/v1/webhooks",
	method: "POST",
	handler: apiHttpAction(
		async ({ ctx, auth, request }) => {
			...
			validateRequiredFields(body, ["name", "url"]);
			...
			return apiResponse(
				...
				{
					...endpoint,
					secret: result.secret, // Only returned once at creation!
				},
				{
					Location: `/api/v1/webhooks?id=${result.endpointId}`,
				},
			);
		},
		{ scope: API_SCOPES.WEBHOOKS_MANAGE },
 ...
 * @scope seal:webhooks:manage
 *
 * @queryparam {string} id - Endpoint ID (required)
 *
 * @returns Webhook endpoint details with statistics
 */
http.route({
	...
	handler: apiHttpAction(
		async ({ ctx, auth, query }) => {
			if (!query.id) {
				throw new ApiError(400, "Endpoint ID is required", "VALIDATION_ERROR");
			}
...
http.route({
	...
	handler: apiHttpAction(
		async ({ ctx, auth, query, request }) => {
			...
			if (
				body.status &&
				!["active", "paused", "disabled"].includes(body.status)
			) {
				throw new ApiError(
					400,
