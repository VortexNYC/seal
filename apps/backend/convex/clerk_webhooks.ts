...
export const syncUser = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingUser) {
      ...
      await ctx.scheduler.runAfter(0, internal.emails.user_email_actions.sendWelcomeEmail, {
...
export const syncOrganization = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingOrg) {
      ...
      const organizationId = await ctx.db.insert("organizations", organizationData);
...
export const removeOrganizationMembership = mutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!organization) {
      throw new ConvexError(`Organization with clerkId ${args.organizationClerkId} not found`);
    }
...
export const upsertMembershipFromClerk = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingByClerkMembership) {
      ...
      console.info(`✅ Synced existing membership with Clerk ID: ${args.clerkMembershipId}`);
    ...
    if (existingByUserOrg) {
      ...
      console.info(`✅ Updated existing membership, added Clerk ID: ${args.clerkMembershipId}`);
...
export const syncMembershipFromClerk = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!membership) {
      console.warn(`⚠️ Membership with Clerk ID ${args.clerkMembershipId} not found for sync`);
      return { synced: false };
    }

    // Just update the timestamp to show it was synced
    // Don't override role or other custom settings
    console.info(`✅ Synced membership with Clerk ID: ${args.clerkMembershipId}`);
...
export const deleteMembershipFromClerk = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!membership) {
      console.warn(`⚠️ Membership with Clerk ID ${args.clerkMembershipId} not found for deletion`);
    ...
    console.info(`✅ Deleted membership with Clerk ID: ${args.clerkMembershipId}`);
...
export const handleInvitationCreated = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (existingInvitation) {
      console.info(`ℹ️ Invitation ${args.clerkInvitationId} already exists, skipping`);
    ...
    const inviter = await ctx.db
      .query("organization_members")
      .withIndex("by_organization", (q) => q.eq("organizationId", organization._id))
      .filter((q) => q.or(q.eq(q.field("role"), "owner"), q.eq(q.field("role"), "admin")))
      .first();

    if (!inviter) {
      console.warn(`⚠️ No owner/admin found for organization ${organization._id}`);
      return { created: false };
    }
    ...
    const expiresAt = args.createdAt ? args.createdAt + expirationMs : Date.now() + expirationMs;
    ...
    if (inviterUser) {
      await ctx.scheduler.runAfter(0, internal.emails.user_email_actions.sendTeamInvitationEmail, {
...
export const handleInvitationAccepted = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!invitation) {
      console.warn(`⚠️ Invitation ${args.clerkInvitationId} not found in database`);
    ...
    if (!user) {
      if (retryCount < maxRetries) {
        ...
        await ctx.scheduler.runAfter(2000, internal.clerk_webhooks.handleInvitationAccepted, {
          ...args,
          retryCount: retryCount + 1,
        });
...
export const handleInvitationRevoked = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (!invitation) {
      console.warn(`⚠️ Invitation ${args.clerkInvitationId} not found for revocation`);
