...
describe("Vortex Billing SaaS processor", () => {
  ...
  test("resolves mapped Vortex account and price configuration", () => {
    const config = resolveVortexBillingConfig(checkoutArgs, {
      ...
      VORTEX_BILLING_ACCOUNT_MAP: JSON.stringify({
        org_seal_123: "bacc_seal_123",
      }),
  ...
  test("uses a direct Vortex price id as the checkout key when no price map exists", () => {
    ...
    expect(config).toEqual({
      ...
      subscriptionExternalId:
        "vtx_sub_seal_org_org_seal_123_vtx_price_seal_pro_monthly",
    });
  });
    ...
    expect(
      resolveVortexBillingPortalConfig(
        { organizationId: "org_seal_123" },
        {
          VORTEX_BILLING_API_BASE_URL: "https://payments.vortex.test",
          VORTEX_BILLING_API_KEY: "vb_test",
          VORTEX_BILLING_CUSTOMER_MAP: JSON.stringify({
            org_seal_123: "vtx_cust_mapped",
          }),
        }
      )
    ).toEqual({
  ...
  test("creates a hosted checkout session through the Vortex API", async () => {
    ...
    if (capturedRequest === undefined) {
      throw new Error(
        "Expected Vortex Billing checkout request to be captured"
      );
    }

    expect(capturedRequest.url).toBe(
      "https://billing.vortex.test/v1/checkout/sessions"
    );
  ...
  test("applies and verifies a Vortex coupon before returning checkout", async () => {
    ...
    const mockFetch: typeof fetch = Object.assign(
      async (
        ...
        if (
          request.url ===
          "https://billing.vortex.test/v1/coupons/coupon_25/apply"
        ) {
          return jsonResponse({
            data: {
              appliedCoupon: {
                appliedCouponId:
                  "seal-saas-coupon:org_seal_123:pro_monthly_v2:coupon_25",
              },
            },
          });
        }
        if (
          request.url === "https://billing.vortex.test/v1/checkout/sessions"
        ) {
          return jsonResponse(
            {
              data: {
  ...
  test("fails closed before checkout when a Vortex coupon code is invalid", async () => {
    ...
    expect(
      capturedRequests.map((request) => `${request.method} ${request.url}`)
    ).toEqual(["GET https://billing.vortex.test/v1/coupons"]);
  });
...
describe("readVortexProductAccess (VOR-67)", () => {
  ...
  const clientReturning = (body: unknown, onUrl?: (url: string) => void) => {
    ...
    return createClient({
      baseUrl: "https://payments.vortex.test",
      fetch: mockFetch,
    });
  };
  ...
  test("entitled=true when the namespaced key is in the customer's active keys", async () => {
    const result = await readVortexProductAccess({
      client: clientReturning({
        data: {
          entitlements: { activeKeys: ["vortex.invoice", "vortex.sign"] },
        },
      }),
      customerExternalId,
      product: "invoice",
  ...
  test("entitled=false when the key is absent from active keys", async () => {
    const result = await readVortexProductAccess({
      client: clientReturning({
        data: { entitlements: { activeKeys: ["vortex.sign"] } },
      }),
      customerExternalId,
      product: "finance",
    });
...
function createCouponCheckoutMock(
  ...
  return Object.assign(
    async (
      ...
      if (
        request.url === "https://billing.vortex.test/v1/coupons/coupon_25/apply"
      ) {
        return jsonResponse({
          data: {
            appliedCoupon: {
