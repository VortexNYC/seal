...
```typescript
const response = await fetch("https://seal.convex.site/api/v1/contacts/get?id=con_abc123", {
  headers: {
    Authorization: "Bearer ak_your_api_key_here",
  },
});
...
```typescript
const response = await fetch("https://seal.convex.site/api/v1/contacts/delete?id=con_abc123", {
  method: "DELETE",
  headers: {
    Authorization: "Bearer ak_your_api_key_here",
  },
...
<Callout type="warn">
  **Warning**: Deleting a contact is permanent. Historical documents that referenced this contact's
  email are not affected — only the contact directory entry is removed.
</Callout>
