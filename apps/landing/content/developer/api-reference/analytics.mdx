...
<Callout type="info">
  If `from` and `to` are omitted, the endpoint defaults to the **last 30 days**. The
  `workspace_snapshot` is always a current point-in-time count, regardless of the date range.
</Callout>
...
const response = await fetch(`https://seal.convex.site/api/v1/analytics?from=${from}&to=${to}`, {
  headers: {
