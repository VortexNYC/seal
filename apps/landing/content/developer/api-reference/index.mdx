...
See the [Authentication Guide](/developer/getting-started/authentication) for details on creating and managing API keys.

## Interactive Reference

Explore all endpoints in the interactive API playground:

<Card title="Open API Playground →" href="/developer/openapi/listDocuments">
  Try every endpoint live — browse schemas, fill in parameters, and run real
  requests against the Seal API.
</Card>
...
<Cards>
  <Card title="Documents" href="/developer/api-reference/documents">
    Upload, send, track, void, and download signed documents
  </Card>
  <Card title="Recipients" href="/developer/api-reference/recipients">
    Manage document recipients with role-based access control
  </Card>
  <Card title="Templates" href="/developer/api-reference/templates">
    Create and manage reusable document templates
  </Card>
  <Card title="Signatures" href="/developer/api-reference/signatures">
    Verify signatures and access audit trails
  </Card>
  <Card title="Contacts" href="/developer/api-reference/contacts">
    Manage your workspace contact directory
  </Card>
  <Card title="Members" href="/developer/api-reference/members">
    List and retrieve workspace team members
  </Card>
  <Card title="Analytics" href="/developer/api-reference/analytics">
    Document completion rates, signing metrics, and workspace trends
  </Card>
  <Card title="Audit Log" href="/developer/api-reference/audit-log">
    Tamper-evident activity history for compliance and security
  </Card>
  <Card title="Settings" href="/developer/api-reference/settings">
    Read and update workspace configuration programmatically
  </Card>
  <Card title="Webhooks" href="/developer/api-reference/webhooks">
    Configure webhook endpoints for real-time notifications
  </Card>
  <Card title="Uploads" href="/developer/api-reference/uploads">
...
The Seal API uses query-parameter-based routing for resource operations. Resource IDs are passed as query parameters:
...
See the [Error Handling Guide](/developer/getting-started/error-handling) for complete error documentation.
...
```typescript
// First page
const page1 = await fetch("https://api.seal.nyc/api/v1/documents?limit=20");
const { data, has_more, next_cursor } = await page1.json();

// Next page
if (has_more) {
  const page2 = await fetch(
    `https://api.seal.nyc/api/v1/documents?limit=20&cursor=${next_cursor}`
  );
}
```
...
- **[Documents API](/developer/api-reference/documents)** - Complete document management endpoints
- **[Quick Start Guide](/developer/getting-started/quick-start)** - Send your first document
- **[Webhooks](/developer/webhooks)** - Set up real-time event notifications
