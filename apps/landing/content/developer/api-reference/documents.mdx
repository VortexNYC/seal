...
<Callout type="warn">
  **Warning**: Deleting a document is permanent and cannot be undone. All associated recipients,
  signatures, and audit trail data will also be deleted.
</Callout>
...
<Callout type="info">
  **Note**: Voiding a document prevents recipients from signing but preserves the document and audit
...
<Callout type="info">
  **Note**: Download URLs are temporary and expire after a short period (typically 1 hour). Generate
  a new URL if the previous one has expired.
</Callout>
...
```typescript
const response = await fetch("https://seal.convex.site/api/v1/documents/bulk-void", {
  method: "POST",
  headers: {
    Authorization: "Bearer ak_your_api_key_here",
    "Content-Type": "application/json",
...
<Callout type="warn">
  Documents that are already `completed`, `cancelled`, or `declined` cannot be voided and will
  appear in `results` with `success: false`.
</Callout>
...
```typescript
const response = await fetch("https://seal.convex.site/api/v1/documents/bulk-send", {
  method: "POST",
  headers: {
    Authorization: "Bearer ak_your_api_key_here",
    "Content-Type": "application/json",
...
<Callout type="info">
  Only `draft` documents with at least one recipient can be bulk-sent. Documents that are already
  sent, completed, or cancelled will appear in `results` with `success: false`.
</Callout>
