...
async function sendDocument(documentId: string, message: string) {
  try {
    const response = await fetch(
      `https://seal.convex.site/api/v1/documents/${documentId}/send`,
      {
        method: "POST",
        headers: {
...
const result = await retryWithBackoff(() =>
  sendDocument("doc123", "Please sign")
);
```
...
<Callout type="info">
  **Tip**: Always log the full error response (including `type` and `instance`)
  to help with debugging. The `instance` field shows exactly which request
  failed.
</Callout>
...
- **[API Reference](/developer/api-reference)** - Explore all endpoints and their specific error conditions
- **[Webhooks](/developer/webhooks)** - Set up error notifications via webhooks
- **[Authentication](/developer/getting-started/authentication)** - Review authentication requirements and scope errors
