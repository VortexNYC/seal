...
test.describe("landing homepage", () => {
  test("renders homepage shell and critical sections without browser errors", async ({
    page,
  }) => {
  ...
  test("desktop navigation reaches public routes and in-page anchors", async ({
    ...
    test.skip(
      isMobile,
      "Desktop navigation assertions are covered in desktop projects."
    );
    ...
    await Promise.all([
      ...
      homePage.desktopNavLink("Docs").click(),
    ]);
    ...
    await Promise.all([
      ...
      homePage.desktopNavLink("Compare").click(),
    ]);
    await expect(page).toHaveURL(/\/compare$/);
    await expect(
      page.getByRole("heading", {
        level: 1,
        name: /we built a document platform/i,
      })
    ).toBeVisible();
  ...
  test("mobile menu opens and navigates correctly", async ({
    page,
    isMobile,
  }) => {
    ...
    await Promise.all([
      page.waitForURL(/\/compare$/),
      homePage.mobileMenuLink("Compare").click(),
    ]);
    await expect(page).toHaveURL(/\/compare$/);
    await expect(
      page.getByRole("heading", {
