    ...
    <Dialog>
      ...
      <DialogContent>
        ...
        <div className="prose prose-sm dark:prose-invert max-w-none mt-2">
          {content.trim().split("\n").map((line, i) => {
            if (line.startsWith("## ")) return <h2 key={i} className="text-foreground font-serif text-xl mt-0 mb-4">{line.slice(3)}</h2>;
            if (line.startsWith("### ")) return <h3 key={i} className="text-foreground font-medium text-base mt-6 mb-2">{line.slice(4)}</h3>;
            if (line.startsWith("- ")) return <li key={i} className="text-muted-foreground text-sm ml-4 list-disc">{parseInline(line.slice(2), i)}</li>;
            if (line === "") return <div key={i} className="h-2" />;
            return <p key={i} className="text-muted-foreground text-sm leading-relaxed">{parseInline(line, i)}</p>;
          })}
        </div>
      </DialogContent>
    </Dialog>
...
function parseInline(text: string, lineIndex: number): React.ReactNode {
  ...
  return parts.map((part, i) =>
    part.startsWith("**") && part.endsWith("**")
      ? <strong key={`${lineIndex}-${i}`} className="text-foreground font-medium">{part.slice(2, -2)}</strong>
      : part
  );
}
...
function FooterNavLink({ label, to, href }: { label: string; to?: string; href?: string }) {
  if (href) {
    return (
      <a className="text-muted-foreground hover:text-foreground text-sm transition-colors" href={href} target="_blank" rel="noreferrer">
        {label}
      </a>
    );
  }
    ...
    <footer className="border-border border-t px-6 py-16" data-testid="site-footer">
      <div className="mx-auto max-w-6xl">
        <div className="grid gap-12 sm:grid-cols-2 md:grid-cols-3" data-testid="site-footer-inner">
          ...
          {/* Legal */}
          <div>
            <p className="text-foreground mb-4 text-xs font-semibold tracking-[0.12em] uppercase">Legal</p>
            <ul className="space-y-3">
              <li>
                <LegalModal
                  ...
                  trigger={
                    <button className="text-muted-foreground hover:text-foreground text-sm transition-colors text-left">
                      Privacy
                    </button>
                  }
                />
