...
const { uploadUrl, storageId } = await uploadResponse.json();
```
...
```json
{
  "uploadUrl": "https://storage.convex.cloud/...",
  "storageId": "kg2h4j5k6l7m8n9p0q1r2s3t"
}
```
...
## Step 3: Create Document with Recipients

Create a document record and add recipients in a single request:
...
const documentResponse = await fetch('https://seal.convex.site/api/v1/documents', {
  ...
  body: JSON.stringify({
    ...
    storageId: storageId, // from Step 1
    recipients: [
      {
...
```typescript
const sendResponse = await fetch(`https://seal.convex.site/api/v1/documents/${document.id}/send`, {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer ak_your_api_key_here',
    'Content-Type': 'application/json'
...
```typescript
const statusResponse = await fetch(`https://seal.convex.site/api/v1/documents/${document.id}`, {
  headers: {
    'Authorization': 'Bearer ak_your_api_key_here'
  }
});
