...
```typescript
const sendResponse = await fetch(`https://seal.convex.site/api/v1/documents/send?id=${document.id}`, {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer ak_your_api_key_here',
    'Content-Type': 'application/json'
...
```typescript
const statusResponse = await fetch(`https://seal.convex.site/api/v1/documents/get?id=${document.id}`, {
  headers: {
    'Authorization': 'Bearer ak_your_api_key_here'
  }
});
