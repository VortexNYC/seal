...
| `last_triggered_at` | string | ISO 8601 last trigger timestamp |
...
```json
{
  ...
  "last_triggered_at": "2024-01-27T11:00:00Z"
}
```
...
const response = await fetch('https://seal.convex.site/api/v1/webhooks/update?id=wh_abc123', {
  ...
  body: JSON.stringify({
    events: ['document.*'],  // Subscribe to all document events
    status: 'active'
  })
});
```
...
```typescript
const response = await fetch('https://seal.convex.site/api/v1/webhooks/rotate-secret?id=wh_abc123', {
  method: 'POST',
  headers: {
    'Authorization': 'Bearer ak_your_api_key_here'
  }
