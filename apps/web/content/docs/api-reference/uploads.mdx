...
2. **Upload File**: POST the file directly to the generated URL
3. **Create Document**: Use the returned `storage_id` to create a document
...
|-------|------|----------|-------------|
| `filename` | string | Yes | Original filename (e.g., "contract.pdf") |
...
const { upload_url, storage_id } = await response.json();
```
...
const { upload_url, storage_id } = await urlResponse.json();
...
| 400 | `INVALID_FILE_TYPE` | File type doesn't match declared content_type |
...
5. **Store Storage ID**: Save the `storage_id` immediately after generation for document creation
...
- **[Documents API](/docs/api-reference/documents)** - Create documents after uploading
- **[Quick Start](/docs/getting-started/quick-start)** - Complete integration tutorial
- **[Error Handling](/docs/getting-started/error-handling)** - Handle upload errors
