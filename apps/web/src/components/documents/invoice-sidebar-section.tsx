    ...
    >
              ...
              {existingInvoice && (
                <Tooltip>
                  <TooltipTrigger asChild>
                    ...
                    >
                      {existingInvoice.status === "paid" && <CheckCircleIcon className="h-3 w-3" />}
                      {statusConfig.label}
      ...
      <CollapsibleContent className="border-t border-slate-100 px-5 pb-5 sm:px-4 sm:pb-4 dark:border-slate-800">
        <div className="mt-4 space-y-4">
          ...
          {/* No Invoice State - Show creation form (only if canEdit) */}
          {!existingInvoice && canUseInvoices && canEdit && (
            <div className="space-y-3">
