...
import { cn, parseApiError, getErrorMessage, clamp } from "./utils";
...
describe("parseApiError", () => {
  describe("permission errors", () => {
    test("detects 'not authorized' message", () => {
      const result = parseApiError(new Error("User is not authorized"));
    ...
    test("detects 'forbidden' message", () => {
      const result = parseApiError(new Error("Forbidden"));
      expect(result.type).toBe("permission");
    });

    test("detects 'insufficient permissions' message", () => {
      const result = parseApiError(new Error("Insufficient permissions"));
      expect(result.type).toBe("permission");
    });

    test("is case-insensitive", () => {
      const result = parseApiError(new Error("ACCESS DENIED"));
      expect(result.type).toBe("permission");
    });
  });
  ...
  describe("subscription errors", () => {
    ...
    test("detects 'upgrade' message", () => {
      const result = parseApiError(new Error("Please upgrade your plan"));
      expect(result.type).toBe("subscription");
    });

    test("detects 'subscription' message", () => {
      const result = parseApiError(new Error("Active subscription required"));
      expect(result.type).toBe("subscription");
    });

    test("preserves original message as userFriendlyMessage", () => {
      const msg = "Upgrade to Professional to unlock this feature";
      const result = parseApiError(new Error(msg));
  ...
  describe("not_found errors", () => {
    test("detects 'not found' message", () => {
      const result = parseApiError(new Error("Document not found"));
      expect(result.type).toBe("not_found");
      expect(result.userFriendlyMessage).toContain("could not be found");
    });

    test("detects 'does not exist' message", () => {
      const result = parseApiError(new Error("Resource does not exist"));
      expect(result.type).toBe("not_found");
    });
  });
  ...
  describe("validation errors", () => {
    test("detects 'invalid' message", () => {
      const result = parseApiError(new Error("Invalid email address"));
      expect(result.type).toBe("validation");
      expect(result.userFriendlyMessage).toBe("Invalid email address");
    });

    test("detects 'required' message", () => {
      const result = parseApiError(new Error("Name is required"));
      expect(result.type).toBe("validation");
    });

    test("detects 'must be' message", () => {
      const result = parseApiError(new Error("Value must be a number"));
      expect(result.type).toBe("validation");
    });

    test("detects 'cannot be' message", () => {
      const result = parseApiError(new Error("Field cannot be empty"));
      expect(result.type).toBe("validation");
    });
  ...
  describe("non-Error inputs", () => {
    test("handles string input", () => {
      const result = parseApiError("not authorized to do this");
      expect(result.type).toBe("permission");
      expect(result.message).toBe("not authorized to do this");
    });

    test("handles ApiError-like object with data string", () => {
      const apiLikeError = {
        data: "Document not found",
        message: "Document not found",
      };
      // parseApiError uses String() for non-Error objects
      const result = parseApiError(apiLikeError);
      expect(result.type).toBe("unknown");
    });
