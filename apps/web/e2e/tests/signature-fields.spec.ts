...
test.describe("Signature Fields - Selection", () => {
	test("should display all field type buttons", async ({
		...
		await expect(
			authenticatedPage.getByRole("button", { name: "Signature" }),
		).toBeVisible();
		await expect(
			authenticatedPage.getByRole("button", { name: "Text" }),
		).toBeVisible();
		await expect(
			authenticatedPage.getByRole("button", { name: "Date" }),
		).toBeVisible();
		await expect(
			authenticatedPage.getByRole("button", { name: "Checkbox" }),
		).toBeVisible();
	});
...
test.describe("Signature Fields - Management", () => {
	test("should display fields list when empty", async ({
		authenticatedPage,
		organizationSlug,
	}) => {
		const documentPage = new DocumentPage(authenticatedPage);
		...
		await expect(
			authenticatedPage.getByText("No fields added yet"),
		).toBeVisible();
	});
	...
	test("should show helpful tip about field placement", async ({
		...
		// Verify tip is shown
		await expect(
			authenticatedPage.getByText(/drag a field onto the PDF/i),
		).toBeVisible();
	});
...
test.describe("Signature Fields - Toolbar Interactions", () => {
	test("should display signature fields toolbar", async ({
		...
		await expect(
			authenticatedPage.getByText("Drag fields onto the document"),
		).toBeVisible();
	});
