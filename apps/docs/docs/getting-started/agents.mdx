...
## Deal writes the document (field bindings)

Do **not** OCR-reconstruct deals on every send. Tag fillable fields with a
`binding_key`, then push structured values:

```bash
# Place a text field bound to a proposal key
seal POST /api/v1/documents/fields field.json
# field.json includes: binding_key "proposal.amount" (etc.)

# Payments / CRM / agent writes the deal into the draft
seal POST /api/v1/documents/apply-bindings <<'EOF'
{"id":"<document_id>","bindings":{"proposal.amount":"12000","proposal.counterparty_name":"Acme"}}
EOF
```

MCP: `seal_create_document_field` / `seal_apply_document_bindings`. Template
fields support the same `binding_key` via `PUT /api/v1/templates/fields/update`.

Seal-side of GitHub #604. Payments→Seal push when a proposal changes is
SEA-43 (Vortex payments backend), not OCR theater.
...
### Human handoff (InteractionSession)

When the agent hits “human must act” (sign, approve), it does **not** forge
the act. Pattern (ADR-004 / SEA-61):

1. `GET /api/v1/documents/interaction?id=<document_id>` → InteractionSession
2. Show or open `url` (CLI: `seal wait --document <id> --open`; MCP:
   `seal_get_interaction` then `seal_wait_interaction`)
3. Human does the one job on the page (“Sign … then close this tab”)
4. Poll until `status` is terminal — agent continues with the JSON result

Same shape should apply across Vortex (Veil unlock, Pile approve): open URL →
one-shot human surface → poll → resume.
...
## Portfolio default (SEA-59 / ADR-005)

**Seal is the only Vortex signing path** for day-to-day company documents
(offers, NDAs, vendor packets, contractor agreements, internal filings that
are not government portals).

- Do **not** create or resume DocuSign / PandaDoc / HelloSign / Adobe Sign
  workflows for Vortex work.
- Use Seal API / MCP / CLI / InteractionSession instead.
- If Seal cannot do it yet, file a SEA ticket — do not reopen a vendor seat.
- Historical DocuSign/PandaDoc **import** adapters are migration-only, not a
  send path.
...
- ADR: `docs/decisions/ADR-004-interaction-sessions-and-host-hitl.md`
- ADR: `docs/decisions/ADR-005-seal-only-vortex-signing-path.md`
