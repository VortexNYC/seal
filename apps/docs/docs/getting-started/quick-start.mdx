...
- An API key with `seal:documents:write` and `seal:recipients:write` scopes ([create one](/docs/getting-started/authentication))
...
const uploadResponse = await fetch(
  "https://api.seal.nyc/api/v1/uploads/generate-url",
  {
    method: "POST",
    headers: {
      Authorization: "Bearer seal_your_api_key_here",
    },
  }
);
...
curl -X POST https://api.seal.nyc/api/v1/uploads/generate-url \
  -H "Authorization: Bearer seal_your_api_key_here"
```
...
- **[Webhooks](/docs/webhooks/setup)** - Close the golden path: get `document.completed` pushed to you
- **[Production checklist](/docs/getting-started/production)** - Keys, audit, email, smoke before real volume
- **[Self-host](/docs/getting-started/self-host)** - Run Seal on your Cloudflare account
- **[Error Handling](/docs/getting-started/error-handling)** - Learn how to handle API errors gracefully
- **[API Reference](/docs/api-reference/documents)** - Explore all document endpoints and options
- **[Templates](/docs/api-reference/templates)** - Create reusable document templates
