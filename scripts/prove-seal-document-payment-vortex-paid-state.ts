...
function stringArrayField(value: JsonObject, field: string): readonly string[] {
  ...
  for (const entry of child) {
    assert(
      typeof entry === "string",
      `Expected ${field} entries to be strings`
    );
    strings.push(entry);
  }
  return strings;
...
async function readVortexApiConfig(
  sealDeployment: string
): Promise<VortexApiConfig | null> {
  const apiKey =
...
async function runSealConvex<T extends Json>(input: {
  ...
  if (result.exitCode !== 0) {
    fail(
      `seal convex run ${input.functionName} failed\n${result.stderr}\n${result.stdout}`
    );
  }
...
async function readVortexPayable(input: {
  ...
  const response = await fetch(
    `${config.apiBaseUrl}/v1/payables/${input.vortexPayableId}`,
    {
      headers: {
  ...
  assert(
    isJsonObject(body),
    "Expected Vortex payable response to be an object"
  );
  if (!response.ok) {
    return {
  ...
  const payable =
    data === undefined ? undefined : optionalObjectField(data, "payable");
  return payable ?? null;
}
...
function assertProcessorLineage(
  record: JsonObject,
  field: string
): readonly JsonObject[] {
  ...
  assert(
    refs.length > 0,
    `Expected ${field} to include Vortex processor lineage`
  );
  for (const ref of refs) {
    assert(
      stringField(ref, "objectId").length > 0,
      `Expected ${field} processor object id`
    );
  }
  return refs;
}

async function readVortexMoneyPath(
  ...
  const settlement = await readVortexSettlementProof(
    input,
    payableProof.invoiceId
  );
...
async function readPaidPayableProof(
  input: VortexMoneyPathInput
): Promise<VortexPayableProof> {
...
async function readInvoiceCollectionProof(
  ...
  assert(
    artifacts !== null,
    `Expected Vortex invoice artifacts for ${payableProof.invoiceId}`
  );
  ...
  assert(
    numberField(succeededAttempt, "collectedAmount") ===
      payableProof.payableTotal,
...
async function readCanonicalPaymentProof(
  ...
  assert(
    payment !== null,
    `Expected canonical Vortex payment ${proof.paymentId}`
  );
  assert(
  ...
  const paymentIntentRefs = assertProcessorLineage(
    paymentIntent,
    "processorIntentRefs"
  );
...
async function readVortexSettlementProof(
  ...
  assert(
    operatorInspection !== null,
    `Expected Vortex operator inspection for ${invoiceId}`
  );
  const settlementSummary = objectField(
    operatorInspection,
    "paymentSettlementSummary"
  );
  const settlementRows = arrayField(
    objectField(operatorInspection, "paymentSettlement"),
    "rows"
  )
    .filter(isJsonObject)
    .map((row) => ({
  ...
  return {
    ...
    settlementEntryCount: numberField(
      settlementSummary,
      "settlementEntryCount"
    ),
    unsettledPaymentCount: numberField(
      settlementSummary,
...
function eventMatchesPaidPayable(
  event: VortexEvent,
  vortexPayableId: string
): boolean {
  if (
    event.eventType !== paidEventType ||
    event.aggregateId !== vortexPayableId
  ) {
    return false;
  }
  ...
  const payableObject =
    payload === null
      ? undefined
      : optionalObjectField(payload, "payableObject");
...
async function attemptVortexWebhookRedrive(input: {
  ...
  const deliveryToDispatch =
    pending ?? deliveries.find((delivery) => delivery.status === "failed");
  if (deliveryToDispatch === undefined) {
    const succeeded = deliveries.find(
  ...
  assert(
    ...
    "Vortex webhook resend did not create a pending delivery"
  );
...
function printFailureDiagnostic(input: {
  ...
  const vortexStatus =
    input.vortexPayable === null
      ? undefined
      : optionalStringField(input.vortexPayable, "status");
  const amountPaid =
    input.vortexPayable === null
      ? undefined
...
function readProofOptions(): ProofOptions {
  return {
    sealDeployment:
      readEnv("SEAL_CONVEX_DEPLOYMENT") ??
    ...
    requireSettled:
      hasFlag("require-settled") ||
      readEnv("SEAL_REQUIRE_VORTEX_SETTLED") === "true",
    vortexPayableId:
...
function readSealPaidStateSnapshot(state: JsonObject): SealPaidStateSnapshot {
  return {
    ...
    documentWorkflowStatus: optionalStringField(
      state,
...
async function ensureSealPaidState(
  ...
  return await retryProjectionFromPaidVortexPayable(
    options,
    state,
    vortexPayable
  );
...
async function retryProjectionFromPaidVortexPayable(
  ...
  const vortexStatus =
    vortexPayable === null
      ? undefined
      : optionalStringField(vortexPayable, "status");
  const amountRemaining =
    vortexPayable === null
      ? undefined
      : optionalNumberField(vortexPayable, "amountRemaining");
  ...
  assertSealProjectedAfterRedrive(
    options,
    finalState,
    vortexPayable,
    webhookRedrive
...
async function readVortexMoneyPathForProof(
  options: ProofOptions
): Promise<VortexMoneyPathProof> {
...
function assertSettlementRequirement(
  options: ProofOptions,
  vortexMoneyPath: VortexMoneyPathProof
) {
  if (!options.requireSettled || vortexMoneyPath.settlement.fullySettled)
    return;
...
async function main(): Promise<void> {
  ...
  const { finalState, webhookRedrive } = await ensureSealPaidState(
    options,
    state,
    vortexPayable
  );
