...
const envSchema = z.object({
  ...
  // API Keys
  RETIRED_PROVIDER_SECRET_KEY: z.string().startsWith("sk_"),
  RETIRED_PROVIDER_WEBHOOK_SECRET: z.string().startsWith("whsec_"),
...
export const getPaymentIntent = createServerFn({ method: "POST" }).handler(async () => {
  const retired_provider = new retired provider(env.RETIRED_PROVIDER_SECRET_KEY);
...
export const publicEnv = {
  ...
  retired_providerPublicKey: process.env.VITE_RETIRED_PROVIDER_PUBLIC_KEY!,
...
declare namespace NodeJS {
  interface ProcessEnv {
    ...
    // Vite public vars
    VITE_APP_URL?: string;
    VITE_RETIRED_PROVIDER_PUBLIC_KEY: string;
  }
}
```
