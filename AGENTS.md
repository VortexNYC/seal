...
* **Rule of two:** if this PR introduces code that already exists in 2 or more Vortex codebases (helpers, gates, scripts, config patterns, provider-specific utilities), REJECT it — the primitive belongs in a published `@vortexnyc/*` package. Flag the duplication, name the repos, and require a Core lift ticket instead of a third copy.
